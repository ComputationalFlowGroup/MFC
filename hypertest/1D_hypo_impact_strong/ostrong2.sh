...
$(printf "$TABLE_FORMAT_LINE" "Job Name"     "ostrong2"                        "Engine"     "interactive")
...
END
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "MFC case # ostrong2 @ /u/mcarcanabarbosa/MFC/examples/1D_hypo_impact_strong/case.py:"
    printf "$TABLE_HEADER"
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/sw/spack/deltas11-2023-03/apps/linux-rhel8-zen3/nvhpc-22.11/openmpi-4.1.5-nzb4n4r/lib/
    ...
    cd "/u/mcarcanabarbosa/MFC/examples/1D_hypo_impact_strong"
    ...
    if [ 'simulation' == 'simulation' ]; then
        export CRAY_ACC_MODULE='/u/mcarcanabarbosa/MFC/build/staging/c4aa7d20bc/simulation-wg256.lld.exe'
    fi

    cd "/u/mcarcanabarbosa/MFC/examples/1D_hypo_impact_strong"

    t_simulation_start=$(perl -MTime::HiRes=time -e 'printf "%.9f\n", time')


        (set -x;                                                mpirun -np 1                                                        "/u/mcarcanabarbosa/MFC/build/install/c4aa7d20bc/bin/simulation")

    
    code=$?

    t_simulation_stop=$(perl -MTime::HiRes=time -e 'printf "%.9f\n", time')

    if [ $code -ne 0 ]; then
        echo
        error ":( $MAGENTA/u/mcarcanabarbosa/MFC/build/install/c4aa7d20bc/bin/simulation$COLOR_RESET failed with exit code $MAGENTA$code$COLOR_RESET."
        echo
        exit 1
    fi
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "Finished ostrong2:"
