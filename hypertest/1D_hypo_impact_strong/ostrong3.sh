...
$(printf "$TABLE_FORMAT_LINE" "Job Name"     "ostrong3"                        "Engine"     "interactive")
...
END
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "MFC case # ostrong3 @ /u/mcarcanabarbosa/MFC/examples/1D_hypo_impact_strong/case.py:"
    printf "$TABLE_HEADER"
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/sw/spack/deltas11-2023-03/apps/linux-rhel8-zen3/nvhpc-22.11/openmpi-4.1.5-nzb4n4r/lib/
    ...
    if [ 'post_process' == 'simulation' ]; then
        export CRAY_ACC_MODULE='/u/mcarcanabarbosa/MFC/build/staging/292f7334c0/simulation-wg256.lld.exe'
    fi
    ...
    t_post_process_start=$(perl -MTime::HiRes=time -e 'printf "%.9f\n", time')
        ...
        (set -x;                                                mpirun -np 1                                                        "/u/mcarcanabarbosa/MFC/build/install/292f7334c0/bin/post_process")
    ...
    if [ $code -ne 0 ]; then
        echo
        error ":( $MAGENTA/u/mcarcanabarbosa/MFC/build/install/292f7334c0/bin/post_process$COLOR_RESET failed with exit code $MAGENTA$code$COLOR_RESET."
        echo
        exit 1
    fi
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "Finished ostrong3:"
