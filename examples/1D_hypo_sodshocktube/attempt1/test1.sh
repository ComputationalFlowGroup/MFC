...
END
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "MFC case # test1 @ /oscar/home/mcarcana/MFC/examples/1D_hypo_sodshocktube/attempt1/case.py:"
    printf "$TABLE_HEADER"
...
echo
    ...
    cd "/oscar/home/mcarcana/MFC/examples/1D_hypo_sodshocktube/attempt1"
    ...
    if [ 'pre_process' == 'simulation' ]; then
        export CRAY_ACC_MODULE='/oscar/home/mcarcana/MFC/build/staging/f3387f1277/simulation-wg256.lld.exe'
    fi
    ...
    t_pre_process_start=$(perl -MTime::HiRes=time -e 'printf "%.9f\n", time')
        ...
        (set -x;                 mpirun -np 1                                                        "/oscar/home/mcarcana/MFC/build/install/f3387f1277/bin/pre_process")
    ...
    if [ $code -ne 0 ]; then
        echo
        error ":( $MAGENTA/oscar/home/mcarcana/MFC/build/install/f3387f1277/bin/pre_process$COLOR_RESET failed with exit code $MAGENTA$code$COLOR_RESET."
        echo
        exit 1
    fi
