...
int_energy = 0.5 * Kt0_tilde * pow(math.log(tilde_rho), 2) * (1 + (Kt0_prime_suc - 2) * math.log(tilde_rho) / 3) + A_tilde * (
    phi * theta_E_tilde * math.exp(phi * theta_E_tilde) / (math.exp(phi * theta_E_tilde) - 1) - math.log(math.exp(phi * theta_E_tilde) - 1)
)
    ...
    json.dumps(
        {
            ...
            "patch_icpp(1)%alpha_rho(1)": 9.82454806e-7,
            ...
            "fluid_pp(1)%qv": 2.0e0,  # K'_theta0 for sucrose
            "fluid_pp(2)%qv": 2.0e0,  #
            ...
            "fluid_pp(2)%mg_b": 1.0e0,  # b_mg
            "fluid_pp(1)%rho0": 9.82e-7,  # Non-dimensional initial density in Birch-Murnaghan cold curve
