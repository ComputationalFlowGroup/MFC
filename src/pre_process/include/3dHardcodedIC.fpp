...
#:def Hardcoded3D()
    ...
    case (302) ! (3D lung geometry in X direction - axisym, with smoothing)
        ...
        intH = 0.5*amp*(sin(2*pi*y_cc(j)/lam + pi/2) + sin(2*pi*z_cc(k)/lam + pi/2))

        alph = patch_icpp(2)%alpha(1) + (patch_icpp(1)%alpha(1)-patch_icpp(2)%alpha(1))/(h)*(x_cc(i)-(intH-h/2))
        
        if (x_cc(i) > intH + h/2.d0) then
        ...
        else if ((x_cc(i) .le. intH + h/2.d0) .and. (x_cc(i) .ge. intH - h/2.d0)) then
