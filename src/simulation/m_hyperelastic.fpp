...
module m_hyperelastic
 ...
 s_finalize_hyperelastic_module
    ...
    !! The btensor_in at the cell-interior Gaussian quadrature points.
    !! These tensor is needed to be calculated once and make the code DRY.
    type(vector_field) :: btensor_in !<
    $:GPU_DECLARE(create='[btensor_in]')
...
contains
    ...
    !>  The following subroutine handles the calculation of the btensor_in.
        !!   The calculation of the btensor_in takes qprimvf.
        !! @param q_prim_vf Primitive variables
        !! @param btensor_in is the output
        !! calculate the grad_xi, grad_xi is a nxn tensor
        !! calculate the inverse of grad_xi to obtain F, F is a nxn tensor
        !! calculate the FFtranspose to obtain the btensor_in, btensor_in is nxn tensor
        !! btensor_in is symmetric, save the data space
    impure subroutine s_initialize_hyperelastic_module
        integer :: i !< generic iterator

        @:ALLOCATE(btensor_in%vf(1:b_size))
        do i = 1, b_size
            @:ALLOCATE(btensor_in%vf(i)%sf(0:m, 0:n, 0:p))
        end do
        @:ACC_SETUP_VFs(btensor_in)
                ...
                do j = 0, m
                    ...
                    if (G_local > verysmall) then
                        ...
                        if (tensorb(tensor_size) > verysmall) then
                            ...
                            ! STEP 3: update the btensor_in, this is consistent with Riemann solvers
                            ! \b_xx
                            btensor_in%vf(1)%sf(j, k, l) = tensorb(1)
                            if (n > 0) then
                                ...
                                #:for BIJ, TXY in [(1,1),(2,2),(3,4)]
                                    btensor_in%vf(${BIJ}$)%sf(j, k, l) = tensorb(${TXY}$)
                                #:endfor
                            end if
                            if (p > 0) then
                                ...
                                ! STEP 3a: update the btensor_in, this is consistent with Riemann solvers
                                #:for BIJ, TXY in [(1,1),(2,2),(3,5),(4,3),(5,6),(6,9)]
                                    btensor_in%vf(${BIJ}$)%sf(j, k, l) = tensorb(${TXY}$)
                                #:endfor
                            end if

                            !STEP 3b: store the determinant at the last entry of the btensor_in
                            btensor_in%vf(b_size)%sf(j, k, l) = tensorb(tensor_size)

                            ! STEP 4a: updating the Cauchy stress primitive scalar field
                            if (hyper_model == 1) then
                                call s_neoHookean_cauchy_solver(btensor_in%vf, q_prim_vf, G_local, j, k, l)
                            elseif (hyper_model == 2) then
                                call s_Mooney_Rivlin_cauchy_solver(btensor_in%vf, q_prim_vf, G_local, j, k, l)
                            end if
