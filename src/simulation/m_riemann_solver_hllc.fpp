            ...
            if (norm_dir == ${NORM_DIR}$) then
                ...
                if (model_eqns == model_eqns_6eq) then
                    ...
                    $:GPU_PARALLEL_LOOP(collapse=3, private='[i, j, k, l, vel_L, vel_R, Re_L, Re_R, alpha_L, alpha_R, &
                                        ...
                                        & qv_R, qv_avg, c_L, c_R, Ca_inv_L, Ca_inv_R, rho_avg, H_avg, c_avg, gamma_avg, ptilde_L, &
                        ...
                        do k = ${Y_BND}$%beg, ${Y_BND}$%end
                            do j = ${X_BND}$%beg, ${X_BND}$%end
                                ...
                                if (hyperelasticity) then
                                    ...
                                    ! Elastic contribution to energy if Ca_inv large enough
                                    if (Ca_inv_L > verysmall .and. Ca_inv_R > verysmall) then
                                        E_L = E_L + Ca_inv_L*qL_prim_rsx_vf(${SF('')}$, eqn_idx%xi%end + 1)
                                        E_R = E_R + Ca_inv_R*qR_prim_rsx_vf(${SF(' + 1')}$, eqn_idx%xi%end + 1)
                                    end if
                                    $:GPU_LOOP(parallelism='[seq]')
                                    do i = 1, b_size - 1
                    ...
                    $:GPU_PARALLEL_LOOP(collapse=3, private='[i, q, alpha_rho_L, alpha_rho_R, vel_L, vel_R, alpha_L, alpha_R, &
                                        ...
                                        & Ca_inv_L, Ca_inv_R, rho_avg, H_avg, c_avg, gamma_avg, ptilde_L, ptilde_R, vel_L_rms, &
                                        ...
                                        & s_M, s_P, s_S, xi_M, xi_P, xi_L, xi_R, xi_L_m1, xi_R_m1, xi_MP, xi_PP, Ys_L, Ys_R, &
                    ...
                    $:GPU_PARALLEL_LOOP(collapse=3, private='[i, T_L, T_R, vel_L_rms, vel_R_rms, pres_L, pres_R, rho_L, gamma_L, &
                                        ...
                                        & h_iL, h_iR, h_avg_2, Ca_inv_L, Ca_inv_R]', copyin='[is1, is2, is3]', &
                                        & firstprivate='[Re_size_loc1, Re_size_loc2]')
                    do l = ${Z_BND}$%beg, ${Z_BND}$%end
                        do k = ${Y_BND}$%beg, ${Y_BND}$%end
                            do j = ${X_BND}$%beg, ${X_BND}$%end
