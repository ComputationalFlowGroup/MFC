...
module m_variables_conversion
    ...
    real(wp), allocatable, dimension(:)   :: Ca_invs_vc
    integer, allocatable, dimension(:)    :: bubrs_vc
    real(wp), allocatable, dimension(:,:) :: Res_vc
    $:GPU_DECLARE(create='[bubrs_vc, Ca_invs_vc, Res_vc]')
...
contains
    ...
    !! procedure pointer.
    subroutine s_convert_to_mixture_variables(q_vf, i, j, k, rho, gamma, pi_inf, qv, Re_K, Ca_inv_K, Ca_inv)
        ...
        if (model_eqns == model_eqns_gamma_law) then  ! Gamma/pi_inf model
            ...
            call s_convert_species_to_mixture_variables(q_vf, i, j, k, rho, gamma, pi_inf, qv, Re_K, Ca_inv_K, Ca_inv)
        end if

    end subroutine s_convert_to_mixture_variables

    !> Compute the pressure from the appropriate equation of state
    subroutine s_compute_pressure(energy, alf, dyn_p, pi_inf, gamma, rho, qv, rhoYks, pres, T, stress, mom, Ca_inv, pres_mag)
        ...
        real(wp), intent(in), optional  :: Ca_inv, pres_mag
    ...
    subroutine s_convert_species_to_mixture_variables_acc(rho_K, gamma_K, pi_inf_K, qv_K, alpha_K, alpha_rho_K, Re_K, Ca_inv_K, &
        ...
        #:if not MFC_CASE_OPTIMIZATION and USING_AMD
            ...
            real(wp), optional, dimension(3), intent(in) :: Ca_inv
        #:else
    ...
    impure subroutine s_initialize_variables_conversion_module
        ...
        @:ALLOCATE(Ca_invs_vc     (1:num_fluids))

        do i = 1, num_fluids
            gammas(i) = fluid_pp(i)%gamma
            gs_min(i) = 1.0_wp/gammas(i) + 1.0_wp
            pi_infs(i) = fluid_pp(i)%pi_inf
            Ca_invs_vc(i) = fluid_pp(i)%Ca_inv
            ps_inf(i) = pi_infs(i)/(1.0_wp + gammas(i))
            cvs(i) = fluid_pp(i)%cv
            qvs(i) = fluid_pp(i)%qv
            qvps(i) = fluid_pp(i)%qvp
        end do
        $:GPU_UPDATE(device='[gammas, gs_min, pi_infs, ps_inf, cvs, qvs, qvps, Ca_invs_vc]')
    ...
    subroutine s_convert_conservative_to_primitive_variables(qK_cons_vf, q_T_sf, qK_prim_vf, ibounds)
        ...
        $:GPU_PARALLEL_LOOP(collapse=3, private='[alpha_K, alpha_rho_K, Re_K, nRtmp, rho_K, gamma_K, pi_inf_K, qv_K, dyn_pres_K, &
                            & rhoYks, B, pres, vftmp, nbub_sc, Ca_inv_K, T, pres_mag, Ga, B2, m2, S, W, dW, E, D, f, dGa_dW, &
                            & dp_dW, df_dW, iter]')
            ...
            do k = ibounds(2)%beg, ibounds(2)%end
                do j = ibounds(1)%beg, ibounds(1)%end
                    ...
                    if (model_eqns /= model_eqns_4eq) then
                        ...
                        if (elasticity) then
                            call s_convert_to_mixture_variables(qK_cons_vf, j, k, l, rho_K, gamma_K, pi_inf_K, qv_K, Re_K, &
                                                                & Ca_inv_K, fluid_pp(:)%Ca_inv)
                        else
        ...
        do l = 0, p
            do k = 0, n
                do j = 0, m
                    ...
                    call s_convert_to_mixture_variables(q_prim_vf, j, k, l, rho, gamma, pi_inf, qv, Re_K, Ca_inv, &
                                                        & fluid_pp(:)%Ca_inv)
                    ...
                    if (hypoelasticity) then
                        if (cont_damage) Ca_inv = Ca_inv*max((1._wp - q_prim_vf(eqn_idx%damage)%sf(j, k, l)), 0._wp)
                        do i = eqn_idx%stress%beg, eqn_idx%stress%end
                            ! adding elastic contribution
                            if (Ca_inv > verysmall) then
                                q_cons_vf(eqn_idx%E)%sf(j, k, l) = q_cons_vf(eqn_idx%E)%sf(j, k, l) + (q_prim_vf(i)%sf(j, k, &
                                          & l)**2._wp)/(4._wp*Ca_inv)
                                ! Double for shear stresses
                                if (any(i == shear_indices)) then
                                    q_cons_vf(eqn_idx%E)%sf(j, k, l) = q_cons_vf(eqn_idx%E)%sf(j, k, l) + (q_prim_vf(i)%sf(j, k, &
                                              & l)**2._wp)/(4._wp*Ca_inv)
    ...
    subroutine s_convert_primitive_to_flux_variables(qK_prim_vf, FK_vf, FK_src_vf, is1, is2, is3, s2b, s3b)
        ...
        $:GPU_PARALLEL_LOOP(collapse=3, private='[alpha_rho_K, vel_K, alpha_K, Re_K, Y_K, rho_K, vel_K_sum, pres_K, E_K, gamma_K, &
                            & pi_inf_K, qv_K, Ca_inv_K, T_K, mix_mol_weight, R_gas]')
        do l = is3b, is3e
            do k = is2b, is2e
                do j = is1b, is1e
                    $:GPU_LOOP(parallelism='[seq]')
...
#ifdef MFC_SIMULATION
        @:DEALLOCATE(gammas, gs_min, pi_infs, ps_inf, cvs, qvs, qvps, Ca_invs_vc)
        if (bubbles_euler) then
            @:DEALLOCATE(bubrs_vc)
        end if
        if (viscous) then
